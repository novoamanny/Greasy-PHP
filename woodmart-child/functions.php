...
// WooCommerce Benefits Page (My Account)
    // Before
...
// 	echo "<p>".__( "<div class='rewards-bar'><h3>Want more rewards?</h3> <button onclick=\"location.href='/join-the-society/'\">Join the Society</button></div>", "wt-smart-coupons-for-woocommerce-pro" )."</p>";
    if (current_user_can('society_member_grease')){
        echo "<p>".__( "<div class='rewards-bar-bar'><h3>Grease Level Benefits</h3>" )."</p>";
    }
    if (current_user_can('society_member_grit')){
        echo "<p>".__( "<div class='rewards-bar-bar'><h3>Grit Level Benefits</h3>" )."</p>";
    }
    ...
    if (have_rows('my_account_benefits', 'option')) { // Check if the repeater field has rows
        ...
        // Add Grit Benefits
        if (current_user_can('society_member_grit') || current_user_can('society_member_glory')){
            if (have_rows('grit_level_benefits', 'option')){
                while (have_rows('grit_level_benefits', 'option')) { // Loop through rows
                the_row(); // Set up the current row
                $item = get_sub_field('benefit'); // Get sub-field value
        ...
        // Add Glory Benefits
        if (current_user_can('society_member_glory')){
            if (have_rows('glory_level_benefits', 'option')){
                while (have_rows('glory_level_benefits', 'option')) { // Loop through rows
                the_row(); // Set up the current row
                $item = get_sub_field('benefit'); // Get sub-field value
    ...
    echo "<p>".__( "<div class='rewards-bar-bar'><h3>Want more rewards?</h3></div>", "wt-smart-coupons-for-woocommerce-pro" )."</p>";
}
